...
実際には反射波などの影響で数メートルの誤差が含まれるデータを効果的に検知・排除する．
...
\begin{figure}[t]
  ...
  \caption{Overview of the system architecture and data flow. 
          The PC-side ROS 2 stack runs RTAB-Map to generate a 
          globally consistent map and pose estimates, while the AGV-side ROS 2 stack 
...
地図全体が歪んでしまう．したがって，本手法ではGNSSの重みを調整するのではなく，採用か遮断かの2通りで制御を行う．
...
\begin{itemize}
  ...
  \item 遮断: データをRTAB-Mapへ送らない，または非常に大きな共分散に置き換えることで，そのデータを実質的に無効化する．
\end{itemize}
...
短い時間（$\Delta T$）の間にロボットがどれだけ移動したかという移動量増分を比較する．
...
(b) Close-up view showing an instantaneous position jump of approximately 1.2 m, 
...
\begin{figure}[tb]
  ...
  (b) Zoomed view showing a similar instantaneous position jump of approximately 1.3 m, 
...
壁や柱が二重になっていないかを見ることで，地図の正しさを判断する．
第2に軌跡の滑らかさを確認する．ロボットの動きとしてあり得ない急激な位置の飛びがないかを確認する．
第3に復帰時の挙動を確認する．GNSSが不安定な状態から回復したときに，
地図が壊れずに維持できているかを確認する．
...
\begin{figure}[htbp]
    ...
    \begin{minipage}[b]{0.48\textwidth}
        ...
        \caption{Zoom-in view around point A (NLOS-affected segment) in Fig.~\ref{fig:traj_global}.}
...
\begin{figure}[tb]
  ...
  \caption{Time history of the trajectory separation between the proposed and naive methods.}
...
\begin{figure}[tb]
  ...
  \begin{subfigure}[t]{0.49\linewidth}
    ...
    \caption{Naive GNSS–LIO fusion (global view).}
  ...
  \begin{subfigure}[t]{0.49\linewidth}
    ...
    \caption{Naive GNSS–LIO fusion (zoom-in view).}
  ...
  \begin{subfigure}[t]{0.49\linewidth}
    ...
    \caption{LIO-only mapping (without GNSS constraint).}
    \label{fig:lio_only}
  \end{subfigure}
  \hfill
  \begin{subfigure}[t]{0.49\linewidth}
    ...
    \caption{Mapping with proposed method.}
    \label{fig:proposed}
  \end{subfigure}

  \caption{
  Mapping results for the 2025-10-23 dataset(white: point cloud map, yellow: estimated trajectory).
(a) Naive GNSS–LIO fusion (global view), where the boxed region indicates the area around the observed anomaly.
(b) Zoom-in view of the boxed region in (a).
(c) LIO-only mapping without GNSS constraints.
(d) Mapping result with the proposed method enabled.
  }
...
The dashed line indicates the threshold $\gamma_{\text{pass}}$, and the top bar shows the PASS/FAIL decision.}
